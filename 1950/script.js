document.addEventListener("DOMContentLoaded", function() {
    ...
    let score = 0;
    let reservoirLevel = 450; // Initial reservoir level
    ...
    let depletionMultiplier = 10; // Adjusted depletion multiplier
    ...
    container.addEventListener("click", function(event) {
        if (!gameEnded && event.target.classList.contains("circle")) {
            ...
            if (score >= 15) { // Check if the player has reached a score of 10 to win
                gameOver(true);
            }
            circleCount--;
            if (circleCount === 0) {
    ...
    function spawnCircle() {
        ...
        circleCount++;
        if (circleCount === 1) {
            reservoirInterval = setInterval(updateReservoir, 800); // Start reservoir depletion if a circle is present
        }
        // Adjust the depletion speed when a circle is added
        depletionSpeed = Math.max(1, circleCount);
    }

    spawnInterval = setInterval(spawnCircle, 2000); // Spawn a circle every 3 seconds
